...
<!-- ```{r}  -->
<!-- pq_stream_sid <- open_dataset("/Volumes/HCUPdata-CC0941-MEDSPH/ParquetData/NY_SEDD", format = "parquet") -->

<!-- ds1 <- pq_stream_sid %>%  -->
<!--   dplyr::select("I10_DX2") %>% -->
<!--   collect() -->
...
pq_stream_sedd_ny <- open_dataset("/Volumes/HCUPdata-CC0941-MEDSPH/ParquetData/NY_SEDD", format = "parquet")
...
#saveRDS(ed_ny,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/ny_ed_rsv.RDS')



##inpatient
pq_stream_sid_ny <- open_dataset("/Volumes/HCUPdata-CC0941-MEDSPH/ParquetData/NY", format = "parquet")
...
#saveRDS(inp_ny,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/ny_inp_rsv.RDS')
```
...
inp_ny <- inp_ny %>%  
  ...
  mutate(AGE = as.numeric(AGE)) %>% 
  mutate(AGE_GROUP = if_else(AGE >=0 & AGE < 1, "<1", 
                             if_else(AGE >=1 & AGE < 2, "1-2",
                                     if_else(AGE >= 2 & AGE <3, "2-3", 
                                             ifelse(AGE >= 3 & AGE < 4, "3-4", 
                                                    ifelse(AGE >= 4 & AGE < 5, "4-5", 
                                                           ifelse(AGE >= 5 & AGE <19, "5-19", 
                                                                  ifelse(AGE >= 19 & AGE <39, "19-39",
                                                                         ifelse(AGE >= 39 & AGE < 59, "39-59", 
                                                                                ifelse(AGE >= 59 & AGE <79, "59-79", ">80+")))))))))) 
inp_ny <- inp_ny %>% 
...
inp_ny %>% 
  filter(date >= "2015-07-01" & date <= "2019-07-01") %>% 
  group_by(date, AGE_GROUP) %>% 
  summarise(N = n()) %>% 
  ggplot() + 
  geom_line(aes(x = date, y = N, group = AGE_GROUP, color = AGE_GROUP)) + 
  ylab("Inpatient")
...
ed_ny %>% 
  ...
  ggplot() + 
  geom_line(aes(x = date, y = N, group = AGE_GROUP, color = AGE_GROUP)) + 
  ylab("Emergence Department")


# X <- bind_rows(inp_ny, ed_ny) %>% 
#   group_by(date) %>% 
#   summarise(N = n()) %>%  
#   mutate(season = ifelse(date >= "2015-07-01" & date <= "2016-07-01", "2015/16", 
#                          ifelse(date >= "2016-07-01" & date <= "2017-07-01", "2016/17",
...
#   filter(!is.na(season)) %>% 
#   ungroup() %>% 
#   mutate(average = rep(X$average, X$number_months),
#          standard_devi = rep(X$standard_devi, X$number_months)) %>% 
#   mutate(scale_cases = (N - average) / standard_devi) # normalization Z-score
...
all_data <- bind_rows(inp_ny, ed_ny) %>%
  ...
  summarize(N=n()) %>% 
  ungroup() %>% 
  mutate(average = ifelse(season == "2015/16", X$average[1],
                          ifelse(season == "2016/17", X$average[2],
                                 ifelse(season == "2017/18", X$average[3], X$average[4])))) %>% 
  mutate(standard_devi = ifelse(season == "2015/16", X$standard_devi[1],
                          ifelse(season == "2016/17", X$standard_devi[2],
                                 ifelse(season == "2017/18", X$standard_devi[3], X$standard_devi[4])))) %>% 
  mutate(Z_score =  (N - average) / standard_devi)
...
# Set your API key:
census_api_key("54a1f67a8ece8505154628934ac7b50b074a45a1", install=T)
...
#saveRDS(zcta_pop,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/NY_pop_2019.RDS')
```
...
# df <- data.frame(season = nyc_map_data$season,
#                  N = nyc_map_data$N,
#                  population = nyc_map_data$population,
#                  ZCTA5CE10 = nyc_map_data$ZCTA5CE10)
# 
# X <- df  %>% 
#   filter(!is.na(season)) %>% 
...
#   ungroup() %>% 
#   group_by(season, ZCTA5CE10) %>% 
#   summarise(average = mean(inc),
#             standard_devi = sd(inc)) %>% 
#   ungroup()
# 
# 
# 
...
p1a <- nyc_map_data%>%
  mutate(inc=N/population*100000) %>% # calculate the incidence ratio
  filter(!is.na(inc) & population!=0) #%>% 
  #mutate(Z_score_inc = X2$Z_score)

p1a <- p1a %>% 
  group_by(season) %>% 
  mutate(cuts = as.integer(cut(ewcdf(inc, weights=population)(inc), seq(0,1,.01))))
...
#all_data2 <- all_data %>% filter(season == "2016/17")
ny_map_data <- ny_zips %>%
  left_join(all_data, by = c("ZCTA5CE10" = "ZIP")) %>%
    left_join(zcta_pop, by = "ZCTA5CE10")
...
#p2a$cuts <- as.integer(cut(ecdf(p2a[,plotvar])(p2a[,plotvar]), seq(0,1,.01)))
...
ggplot(data = p2a) +
 ...
 # scale_fill_viridis_c(name = "Value") +
  theme_minimal() +
  labs(title = "NY state ZIP Code Choropleth Map")+
    theme(panel.background = element_rect(color = "white", fill = "white")) +
    scale_fill_stepsn('Rate', breaks= 1:100, limits = c(0,100), labels=plotlabs,
                      colors=inferno(20),
                      guide = guide_colorsteps(even.steps = FALSE)) +
...
pq_stream_sedd_fl <- open_dataset("/Volumes/HCUPdata-CC0941-MEDSPH/ParquetData/FL_SEDD", format = "parquet")

ed_fl <- pq_stream_sedd_fl %>%
    dplyr::select(AGE,ZIP, YEAR,ZIP3,ZIPINC_QRTL,starts_with('I10_DX')) %>%
  mutate(   
             all_dx = paste(I10_DX1,I10_DX2,I10_DX3,
                            I10_DX4,
                            I10_DX5,
                            ...
                            I10_DX8,
                            I10_DX9,
                            I10_DX10) # does not have DX11 - 25
                            # I10_DX11,
                            # I10_DX12,
                            # I10_DX13,
                            # I10_DX14,
   ...
   filter(grepl("B974|J121|J205|J210", all_dx)) %>%
    dplyr::select(AGE,ZIP,YEAR, ZIP3,ZIPINC_QRTL,all_dx) %>%
  collect()

#saveRDS(ed_fl,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/fl_ed_rsv.RDS')



##inpatient
pq_stream_inp_fl <- open_dataset("/Volumes/HCUPdata-CC0941-MEDSPH/ParquetData/FL", format = "parquet")

inp_fl <- pq_stream_inp_fl %>%
  # rename(AGE='Age in years at admission',
  #        AYEAR='Admission year',
  #        AMONTH='Admission month',
  #       ZIP='Patient ZIP Code',
...
#saveRDS(inp_fl,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/fl_inp_rsv.RDS')


all_data <- bind_rows(inp_fl, ed_fl) %>%
  group_by(ZIP) %>%
  summarize(N=n())
...
#saveRDS(zcta_pop,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/FL_pop_2019.RDS')
 

options(tigris_use_cache = TRUE)
# Get ZIP code shapefile for New York
fl_zips <- zctas(state = "FL", year = 2010)
...
ggplot(data = p2a) +
 ...
 # scale_fill_viridis_c(name = "Value") +
  theme_minimal() +
  labs(title = "FL state ZIP Code Choropleth Map")+
    theme(panel.background = element_rect(color = "white", fill = "white")) +
    scale_fill_stepsn('Rate', breaks= 1:100, limits = c(0,100), labels=plotlabs,
                      colors=inferno(20),
                      guide = guide_colorsteps(even.steps = FALSE))  
