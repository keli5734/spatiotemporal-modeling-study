---
title: "Incidence ratio"
author: "Ke Li"
date: "2024-08-22"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(arrow)
library(tidyr)
library(dplyr)
library(ggplot2)
library(spatstat)

library(usmap)
library(scales)
#library(spatstat)

vircol <-  viridis_pal()(100)

library(duckdb)
library(plotly)
library(lubridate)
# source('./R/plot.fun.R')
# source('./R/schema.R')
state_cw <- read.csv('./Data/SSA_state_crosswalk.csv')
# state_cw <- statepop %>% select(fips, abbr)
state.info <- cbind.data.frame(state.name, state.abb, state.region)

```


```{r, upload data, warning=FALSE}
inp_ny <- readRDS("/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/ny_inp_rsv.RDS")
ed_ny <- readRDS('/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/ny_ed_rsv.RDS') %>%
  mutate(AYEAR=as.numeric(AYEAR))

inp_fl <- readRDS("/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/fl_rsv.RDS")
ed_fl <- readRDS("/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/fl_ed_rsv.RDS")

ny_data <- rbind(inp_ny, ed_ny)
fl_data <- rbind(inp_fl, ed_fl) # Florida does not have AMONTH 

rm(inp_ny, ed_ny, inp_fl, ed_fl)

```


```{r, adding age group and seasons, warning= FALSE}

ny_data <- ny_data %>% 
  filter(!is.na(AYEAR)) %>% 
  filter(!is.na(AMONTH)) %>% 
  mutate(date = paste0(AYEAR,"/",AMONTH,"/","01")) %>% 
  mutate(date = ymd(date)) %>% 
  mutate(AGE = as.numeric(AGE)) %>% 
  filter(!is.na(AGE)) %>% 
   mutate(AGE_GROUP = ifelse(AGE >=0 & AGE <5, "under 5", 
                            ifelse(AGE >= 5 & AGE <=9, "5-9", 
                                   ifelse(AGE >=10 & AGE <=14, "10-14", 
                                          ifelse(AGE >= 15 & AGE <=19, "15-19",
                                                 ifelse(AGE >=20 & AGE <24, "20-24",
                                                        ifelse(AGE >= 25 & AGE <= 29, "25-29",
                                                               ifelse(AGE >= 30 & AGE <=34, "30-34",
                                                                      ifelse(AGE >= 35 & AGE <= 39, "35-39",
                                                                             ifelse(AGE >= 40 & AGE <=44, "40-44",
                                                                                    ifelse(AGE >= 45 & AGE <=49, "45-49",
                                                                                           ifelse(AGE >= 50 & AGE <= 54, "50-54",
                                                                                                  ifelse(AGE >= 55 & AGE <= 59, "55-59",
                                                                                                         ifelse(AGE >= 60 & AGE <=64, "60-64",
                                                                                                                ifelse(AGE >=65 & AGE <= 69,"65-69",
                                                                                                                       ifelse(AGE >=70 & AGE <= 74, "70-74",
                                                                                                                              ifelse(AGE >=75 & AGE <= 79, "75-79", 
                                                                                                                                     ifelse(AGE >= 80 & AGE <= 84, "80-84",
                                                                                                                                            ifelse(AGE >=85, "85 above", NA))))))))))))))))))) %>% 
filter(!is.na(AGE_GROUP)) %>% 
  mutate(season = ifelse(date >= "2015-07-01" & date <= "2016-07-01", "2015/16",
                         ifelse(date >= "2016-07-01" & date <= "2017-07-01", "2016/17",
                                ifelse(date >= "2017-07-01" & date <= "2018-07-01", "2017/18",
                                       ifelse(date >= "2018-07-01" & date <= "2019-07-01", "2018/19", NA)))))

ny_data$AGE_GROUP <- factor(ny_data$AGE_GROUP, levels = c("under 5", 
                                                          "5-9", 
                                                          "10-14", 
                                                          "15-19", 
                                                          "20-24", 
                                                          "25-29",
                                                          "30-34",
                                                          "35-39",
                                                          "40-44",
                                                          "45-49",
                                                          "50-54",
                                                          "55-59",
                                                          "60-64",
                                                          "65-69",
                                                          "70-74",
                                                          "75-79",
                                                          "80-84",
                                                          "85 above"))

ny_data %>% 
  group_by(AGE_GROUP) %>% 
  summarise(N = n()) %>% 
  ggplot(aes(x = AGE_GROUP, y = N/100000, fill = AGE_GROUP)) + 
  geom_bar(stat = "identity") + 
  theme_minimal() + 
  labs(title = "RSV Incidence by Age Group (NY)", x = "Age Group", y = "Incidence per 100000") + 
  theme(
    plot.title = element_text(size = 16, face = "bold"),       # Title size
    axis.title.x = element_text(size = 14),                    # X-axis label size
    axis.title.y = element_text(size = 14),                    # Y-axis label size
    axis.text.x = element_text(size = 12),                     # X-axis tick size
    axis.text.y = element_text(size = 12)                      # Y-axis tick size
  )





fl_data <- fl_data %>% 
  mutate(AGE = as.numeric(AGE)) %>% 
  filter(!is.na(AGE)) %>% 
  mutate(AGE_GROUP = ifelse(AGE >=0 & AGE <5, "under 5", 
                            ifelse(AGE >= 5 & AGE <=9, "5-9", 
                                   ifelse(AGE >=10 & AGE <=14, "10-14", 
                                          ifelse(AGE >= 15 & AGE <=19, "15-19",
                                                 ifelse(AGE >=20 & AGE <24, "20-24",
                                                        ifelse(AGE >= 25 & AGE <= 29, "25-29",
                                                               ifelse(AGE >= 30 & AGE <=34, "30-34",
                                                                      ifelse(AGE >= 35 & AGE <= 39, "35-39",
                                                                             ifelse(AGE >= 40 & AGE <=44, "40-44",
                                                                                    ifelse(AGE >= 45 & AGE <=49, "45-49",
                                                                                           ifelse(AGE >= 50 & AGE <= 54, "50-54",
                                                                                                  ifelse(AGE >= 55 & AGE <= 59, "55-59",
                                                                                                         ifelse(AGE >= 60 & AGE <=64, "60-64",
                                                                                                                ifelse(AGE >=65 & AGE <= 69,"65-69",
                                                                                                                       ifelse(AGE >=70 & AGE <= 74, "70-74",
                                                                                                                              ifelse(AGE >=75 & AGE <= 79, "75-79", 
                                                                                                                                     ifelse(AGE >= 80 & AGE <= 84, "80-84",
                                                                                                                                            ifelse(AGE >=85, "85 above", NA))))))))))))))))))) %>% 
filter(!is.na(AGE_GROUP))
  

fl_data$AGE_GROUP <- factor(fl_data$AGE_GROUP, levels = c("under 5", 
                                                          "5-9", 
                                                          "10-14", 
                                                          "15-19", 
                                                          "20-24", 
                                                          "25-29",
                                                          "30-34",
                                                          "35-39",
                                                          "40-44",
                                                          "45-49",
                                                          "50-54",
                                                          "55-59",
                                                          "60-64",
                                                          "65-69",
                                                          "70-74",
                                                          "75-79",
                                                          "80-84",
                                                          "85 above"))

fl_data %>% 
  group_by(AGE_GROUP) %>% 
  summarise(N = n()) %>% 
  ggplot(aes(x = AGE_GROUP, y = N/1e+5, fill = AGE_GROUP)) + 
  geom_bar(stat = "identity") + 
  theme_minimal() + 
  labs(title = "RSV Incidence by Age Group (FL)", x = "Age Group", y = "Incidence per 100000") + 
  theme(
    plot.title = element_text(size = 16, face = "bold"),       # Title size
    axis.title.x = element_text(size = 14),                    # X-axis label size
    axis.title.y = element_text(size = 14),                    # Y-axis label size
    axis.text.x = element_text(size = 12),                     # X-axis tick size
    axis.text.y = element_text(size = 12)                      # Y-axis tick size
  )


 
  
```


```{r, import population NY, FL and standarded US 2020, warning=FALSE}
# population by zip code
ny_pop <- readRDS("/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/NY_pop_2019.RDS")
fl_pop <- readRDS("/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/FL_pop_2019.RDS")

standard_popluation <- readxl::read_xlsx(path = "/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/US_2020_population_standarded.xlsx", sheet = "Data") %>%
  mutate(AGE = unique(sort(ny_data$AGE_GROUP))) %>% 
  rename("AGE_GROUP" = "AGE",
         "Fraction" = "Percentage") 

  
ny_data_age_dist <- ny_data %>% 
  group_by(AGE_GROUP, ZIP) %>% 
  summarise(Count = n()) %>% 
  ungroup()  # this gives the number of cases by ZIP-code by AGE_GROUP for NY


fl_data_age_dist <- fl_data %>% 
  group_by(AGE_GROUP, ZIP) %>% 
  summarise(Count = n())  %>% 
  ungroup()  # this gives the number of cases by ZIP-code by AGE_GROUP for FL

  #right_join(standard_popluation) %>% 
  #select(-Population) %>% 
  #group_by(ZIP, AGE_GROUP) 
  #mutate(incidence_rate = Count/population*100000) # take calculate incidence rate here...


AGE_GROUP <- sort(unique(ny_data$AGE_GROUP))
Fraction <- standard_popluation$Fraction
# incidence_rate <- fl_data_age_dist$incidence_rate

 

# n_age_group <- as.numeric(table(ny_data_age_dist$AGE_GROUP))
# ny_data_age_dist$incidence_rate <- rep(incidence_rate,n_age_group)

```

```{r, upload zip code NY, warning=FALSE}

library(sf)
library(ggplot2)
library(dplyr)
library(tigris)
library(viridisLite)

nyc_zips <- c(
  "100", "101", "102", "103", "104", "105", "106", "107", "108", "109", # Manhattan
  "110", "111", "112", "113", "114", "116", # Bronx
   "115", "117", "118", "119", # Staten Island
  "111", "112", "113", "114", "116", # Brooklyn (some overlap with Bronx)
  "110", "111", "113", "114", "116"  # Queens (some overlap with Brooklyn and Bronx)
)


options(tigris_use_cache = TRUE)
# Get ZIP code shapefile for New York
ny_zips <- zctas(state = "NY", year = 2010) # Geographic information for NY

ny_zips <- ny_zips  %>% filter(substr(ZCTA5CE10, 1, 3) %in% nyc_zips) # filter for the NYC only 

zcta_pop <- readRDS('/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/NY_pop_2019.RDS') %>%
  select(GEOID, population = estimate) %>%
  rename(ZCTA5CE10 = GEOID) # give population for the ZIP code level

ny_map_data <- ny_zips %>%
  left_join(ny_data_age_dist, by = c("ZCTA5CE10" = "ZIP")) %>%
    left_join(zcta_pop, by = "ZCTA5CE10") 

ny_map_data <- ny_map_data %>% 
  mutate(fraction  = ifelse(AGE_GROUP == AGE_GROUP[1],Fraction[1],
                            ifelse(AGE_GROUP == AGE_GROUP[2], Fraction[2],
                                   ifelse(AGE_GROUP == AGE_GROUP[3], Fraction[3], 
                                          ifelse(AGE_GROUP == AGE_GROUP[4], Fraction[4], 
                                                 ifelse(AGE_GROUP == AGE_GROUP[5], Fraction[5], 
                                                        ifelse(AGE_GROUP == AGE_GROUP[6], Fraction[6], 
                                                               ifelse(AGE_GROUP == AGE_GROUP[7], Fraction[7],
                                                                      ifelse(AGE_GROUP == AGE_GROUP[8], Fraction[8], 
                                                                             ifelse(AGE_GROUP == AGE_GROUP[9], Fraction[9],
                                                                                    ifelse(AGE_GROUP == AGE_GROUP[10], Fraction[10],
                                                                                           ifelse(AGE_GROUP == AGE_GROUP[11], Fraction[11],
                                                                                                  ifelse(AGE_GROUP == AGE_GROUP[12], Fraction[12],
                                                                                                         ifelse(AGE_GROUP == AGE_GROUP[13], Fraction[13],
                                                                                                                ifelse(AGE_GROUP == AGE_GROUP[14], Fraction[14],
                                                                                                                       ifelse(AGE_GROUP == AGE_GROUP[15], Fraction[15],
                                                                                                                              ifelse(AGE_GROUP == AGE_GROUP[16], Fraction[16], 
                                                                                                                                     ifelse(AGE_GROUP==AGE_GROUP[17], Fraction[17], Fraction[18]))))))))))))))))))
  

incidence_data <- ny_map_data %>% 
  mutate(inc = Count / population * 100000 * fraction) %>% 
  as.data.frame() %>% 
  select(ZCTA5CE10, inc) %>% 
  group_by(ZCTA5CE10) %>% 
  summarise(adj_inc = sum(inc)) %>% 
  ungroup() %>% 
  filter(!is.na(adj_inc))

ny_map_data <- ny_zips %>%
   left_join(incidence_data, by = c("ZCTA5CE10" = "ZCTA5CE10")) %>% 
   left_join(zcta_pop, by = "ZCTA5CE10")


p1a <- ny_map_data%>%
  filter(!is.na(adj_inc))  %>% 
  filter(!is.na(population))

invalid_weights <- is.na(p1a$adj_inc) | is.nan(p1a$adj_inc) | is.infinite(p1a$adj_inc)
p1a <- p1a[!invalid_weights, ]


p1a <- p1a %>% 
  #group_by(AGE_GROUP) %>% 
  mutate(cuts = as.integer(cut(ewcdf(adj_inc, weights=population)(adj_inc), seq(0,1,.01))))

#p1a$cuts = as.integer(cut(ewcdf(p1a$inc, weights=p1a$population)(p1a$inc), seq(0,1,.01)))

roundscale <- 1

quants <- weighted.quantile(p1a$adj_inc, w=p1a$population, probs=seq(0,1,0.25), na.rm = TRUE)

plotlabs <- c(as.character(''), 
                  rep('', 23),  
                  as.character(round(quants[2],roundscale)),
                  rep('', 24),  
                  as.character(round(quants[3],roundscale)),
                  rep('', 24),  
                  as.character(round(quants[4],roundscale)),
                  rep('', 24),  
                  as.character(''))


# p1a %>% filter(AGE_GROUP %in% c("under 5", "5-9", "10-14",
#                                 "60-64","65-69","70-74", "75-79",
#                                 "80-84","85 above")) %>% 
p1a %>% 
ggplot() +
  geom_sf(aes(fill = cuts),color=NA) +
 # scale_fill_viridis_c(name = "Value") +
  theme_minimal() +
  labs(title = "New York City ZIP Code Choropleth Map (2016-2020)")+
    theme(panel.background = element_rect(color = "white", fill = "white")) +
    scale_fill_stepsn('Incidence rate', breaks= 1:100, limits = c(0,100), labels=plotlabs,
                      colors=inferno(20),
                      guide = guide_colorsteps(even.steps = FALSE)) #+ 
  #facet_wrap(~AGE_GROUP)

```


```{r, heatmap FL, warning= FALSE}
# fl_data_age_dist

# zcta_pop <- get_acs(
#   geography = "zcta",
#   variables = "B01003_001", # Total population
#   state = "FL",
#   year = 2019  # You can change the year as needed
# )%>%
#   select(GEOID, population = estimate) %>%
#   rename(ZCTA5CE10 = GEOID) # forward simulation ...
# 
# #saveRDS(zcta_pop,'/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/FL_pop_2019.RDS')
#  
# 
# options(tigris_use_cache = TRUE)
# # Get ZIP code shapefile for New York
# fl_zips <- zctas(state = "FL", year = 2010)
# 
# fl_case_data <- fl_data %>% 
#   group_by(ZIP, AGE_GROUP) %>% 
#   summarise(N = n()) %>% 
#   ungroup()
# 
# #all_data2 <- all_data %>% filter(season == "2016/17")
# fl_map_data <- fl_zips %>%
#   left_join(fl_case_data, by = c("ZCTA5CE10" = "ZIP")) %>%
#     left_join(zcta_pop, by = "ZCTA5CE10")
# 
# 
# 
# p2a <- fl_map_data%>%
#   mutate(inc=N/population*100000) %>%
#   filter(!is.na(inc) & population!=0) 
# 
# p2a <- p2a %>% 
#   group_by(AGE_GROUP) %>% 
#   mutate(cuts = as.integer(cut(ewcdf(inc, weights=population)(inc), seq(0,1,.01))))
# 
# #p2a$cuts <- as.integer(cut(ecdf(p2a[,plotvar])(p2a[,plotvar]), seq(0,1,.01)))
# #p2a$cuts = as.integer(cut(ewcdf(p2a$inc, weights=p2a$population)(p2a$inc), seq(0,1,.01)))
# #p2a$cuts = as.integer(cut(ewcdf(p2a$inc)(p2a$inc), seq(0,1,.01)))
#   
# roundscale <- 1
# #quants <- weighted.quantile(p2a$inc, w=p2a$population, probs=seq(0,1,0.25), na.rm = TRUE)
# quants <- quantile(p2a$inc,  probs=seq(0,1,0.25), na.rm = TRUE)
# 
# plotlabs <- c(as.character(''), 
#                   rep('', 23),  
#                   as.character(round(quants[2],roundscale)),
#                   rep('', 24),  
#                   as.character(round(quants[3],roundscale)),
#                   rep('', 24),  
#                   as.character(round(quants[4],roundscale)),
#                   rep('', 24),  
#                   as.character(''))
#   
# p2a %>% filter(AGE_GROUP %in% c("under 5", "5-9", "10-14",
#                                 "60-64","65-69","70-74", "75-79",
#                                 "80-84","85 above")) %>% 
# ggplot() +
#   geom_sf(aes(fill = cuts),color=NA) +
#  # scale_fill_viridis_c(name = "Value") +
#   theme_minimal() +
#   labs(title = "Florida ZIP Code Choropleth Map by age group (2015-2019)")+
#     theme(panel.background = element_rect(color = "white", fill = "white")) +
#     scale_fill_stepsn('Incidence rate', breaks= 1:100, limits = c(0,100), labels=plotlabs,
#                       colors=inferno(20),
#                       guide = guide_colorsteps(even.steps = FALSE)) + 
#   facet_wrap(~AGE_GROUP)


options(tigris_use_cache = TRUE)
# Get ZIP code shapefile for New York
fl_zips <- zctas(state = "FL", year = 2010) # Geographic information for NY

 

zcta_pop <- readRDS('/Volumes/HCUPdata-CC0941-MEDSPH/Rcode_Ke/FL_pop_2019.RDS') %>%
  select(GEOID, population = estimate) %>%
  rename(ZCTA5CE10 = GEOID) # give population for the ZIP code level

fl_map_data <- fl_zips %>%
  left_join(fl_data_age_dist, by = c("ZCTA5CE10" = "ZIP")) %>%
    left_join(zcta_pop, by = "ZCTA5CE10") 

fl_map_data <- fl_map_data %>% 
  mutate(fraction  = ifelse(AGE_GROUP == AGE_GROUP[1],Fraction[1],
                            ifelse(AGE_GROUP == AGE_GROUP[2], Fraction[2],
                                   ifelse(AGE_GROUP == AGE_GROUP[3], Fraction[3], 
                                          ifelse(AGE_GROUP == AGE_GROUP[4], Fraction[4], 
                                                 ifelse(AGE_GROUP == AGE_GROUP[5], Fraction[5], 
                                                        ifelse(AGE_GROUP == AGE_GROUP[6], Fraction[6], 
                                                               ifelse(AGE_GROUP == AGE_GROUP[7], Fraction[7],
                                                                      ifelse(AGE_GROUP == AGE_GROUP[8], Fraction[8], 
                                                                             ifelse(AGE_GROUP == AGE_GROUP[9], Fraction[9],
                                                                                    ifelse(AGE_GROUP == AGE_GROUP[10], Fraction[10],
                                                                                           ifelse(AGE_GROUP == AGE_GROUP[11], Fraction[11],
                                                                                                  ifelse(AGE_GROUP == AGE_GROUP[12], Fraction[12],
                                                                                                         ifelse(AGE_GROUP == AGE_GROUP[13], Fraction[13],
                                                                                                                ifelse(AGE_GROUP == AGE_GROUP[14], Fraction[14],
                                                                                                                       ifelse(AGE_GROUP == AGE_GROUP[15], Fraction[15],
                                                                                                                              ifelse(AGE_GROUP == AGE_GROUP[16], Fraction[16], 
                                                                                                                                     ifelse(AGE_GROUP==AGE_GROUP[17], Fraction[17], Fraction[18]))))))))))))))))))
  

incidence_data <- fl_map_data %>% 
  mutate(inc = Count / population * 100000 * fraction) %>% 
  as.data.frame() %>% 
  select(ZCTA5CE10, inc) %>% 
  group_by(ZCTA5CE10) %>% 
  summarise(adj_inc = sum(inc)) %>% 
  ungroup() %>% 
  filter(!is.na(adj_inc))

fl_map_data <- fl_zips %>%
   left_join(incidence_data, by = c("ZCTA5CE10" = "ZCTA5CE10")) %>% 
   left_join(zcta_pop, by = "ZCTA5CE10")

 
p2a <- fl_map_data%>%
  filter(!is.na(adj_inc))  %>% 
  filter(!is.na(population))

invalid_weights <- is.na(p2a$adj_inc) | is.nan(p2a$adj_inc) | is.infinite(p2a$adj_inc)
p2a <- p2a[!invalid_weights, ]


p2a <- p2a %>% 
  #group_by(AGE_GROUP) %>% 
  mutate(cuts = as.integer(cut(ewcdf(adj_inc, weights=population)(adj_inc), seq(0,1,.01))))

#p1a$cuts = as.integer(cut(ewcdf(p1a$inc, weights=p1a$population)(p1a$inc), seq(0,1,.01)))

roundscale <- 1

quants <- weighted.quantile(p2a$adj_inc, w=p2a$population, probs=seq(0,1,0.25), na.rm = TRUE)

plotlabs <- c(as.character(''), 
                  rep('', 23),  
                  as.character(round(quants[2],roundscale)),
                  rep('', 24),  
                  as.character(round(quants[3],roundscale)),
                  rep('', 24),  
                  as.character(round(quants[4],roundscale)),
                  rep('', 24),  
                  as.character(''))


p2a %>% 
ggplot() +
  geom_sf(aes(fill = cuts),color=NA) +
 # scale_fill_viridis_c(name = "Value") +
  theme_minimal() +
  labs(title = "Florida ZIP Code Choropleth Map (2016-2020)")+
    theme(panel.background = element_rect(color = "white", fill = "white")) +
    scale_fill_stepsn('Incidence rate', breaks= 1:100, limits = c(0,100), labels=plotlabs,
                      colors=inferno(20),
                      guide = guide_colorsteps(even.steps = FALSE)) #+ 
  #facet_wrap(~AGE_GROUP)

  
```


 